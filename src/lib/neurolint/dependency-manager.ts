...
export class LayerDependencyManager {
  private static readonly DEPENDENCIES = {
    ...
    5: [1, 2, 3, 4], // Next.js depends on all base layers
    6: [1, 2, 3, 4, 5], // Testing depends on all previous
    7: [1, 2, 3, 4, 5, 6], // Adaptive learning depends on all
  };
  ...
  static suggestLayers(code: string): {
    ...
    if (code.includes("useState") && !code.includes("'use client'")) {
      recommended.push(5);
      reasons.push("Next.js App Router fixes needed for client components");
    }

    // Check for performance opportunities
    if (code.includes("function ") && !code.includes("React.memo")) {
      recommended.push(6);
      reasons.push("Performance optimizations available");
    }
